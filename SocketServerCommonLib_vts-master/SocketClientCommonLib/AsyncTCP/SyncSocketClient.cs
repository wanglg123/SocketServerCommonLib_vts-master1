    ...
    public class SyncSocketClient
    {
        ...
        public void Connect(string Serverip, int Port)
        {
            try
            {
                ...
                //  m_tcpClient = new TcpClient();//add by heirui
                m_tcpClient = new TcpClient();
                m_tcpClient.Client.Blocking = true;
                // m_tcpClient.Client.Bind(new IPEndPoint(IPAddress.Parse(Serverip), 6001));
        ...
        /// <param name="commandText">信息</param>
        /// <returns>bool</returns>
        public bool SendData(string commandText)
        {
        ...
        /// <param name="commandText">信息</param>
        /// <returns>bool</returns>
        public bool SendDataByte( byte[] bufferUTF8,int length )
        {
            m_tcpClient.Client.Send(bufferUTF8, 0, length, SocketFlags.None);
           // byte[] bufferUTF8 = Encoding.UTF8.GetBytes(commandText);
            //    int totalLength = sizeof(int) + bufferUTF8.Length; //获取总大小
            //UserToken.SendBuffer.StartPacket();
            ////  UserToken.SendBuffer.m_dynamicBufferManager.WriteInt(totalLength); //写入总大小
